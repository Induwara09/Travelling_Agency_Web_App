...
export default function DestinationDetailsPage() {
  ...
  const customImage = destination.imageUrl && !/commons\.wikimedia\.org|upload\.wikimedia\.org/i.test(destination.imageUrl);
  const photoSource = resolvedPhoto?.sourceUrl || (customImage ? destination.imageSourceUrl : null);

  return (
    <>
      <section className="detail-hero destination-detail-v3">
        <DestinationImage destination={destination} fallback={fallback} alt={destination.name} onResolved={setResolvedPhoto} />
