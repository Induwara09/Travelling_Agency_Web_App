...
PLACE_IMAGES = {
    ...
    "Bambarakanda Falls": "Bambarakanda Falls,Sri Lanka.jpg",
    "Diyaluma Falls": "Diyaluma Falls (Koslanda, Sri Lanka).jpg",
    "Dunhinda Falls": "Dunhinda falls Srilanka.jpg",
}
...
def parse(markdown: str) -> list[dict]:
    ...
    for raw in markdown.splitlines():
        ...
        # Only store a URL when the file is confirmed for this exact place.
        # Other destinations are resolved by the frontend's Commons search
        # component instead of reusing a misleading district-level photo.
...
def write_sql(records: list[dict], output: Path) -> None:
    ...
    for row in records:
        columns = "name, location, district, category, short_description, description, image_url, image_source_url, image_mode, tags, featured, created_at"
        values = [
            row["name"], row["location"], row["district"], row["category"],
            row["shortDescription"], row["description"], row["imageUrl"],
            row["imageSourceUrl"], row["imageMode"], row["tags"], 1 if row["featured"] else 0,
        ]
